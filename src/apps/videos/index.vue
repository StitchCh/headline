...
<div class="abs bg-f flex app-videos">
  <af-left ref="afLeft" color="#4d6d7b" bg-color="#e0eff3" vibrant-color="#0093d4" title="视频" :defaultActive="defaultActive">
    <navigator-item icon="folder" index="all-all" @click="$router.replace('/video/list?status=all')">全部</navigator-item>
    ...
    <navigator-item icon="check_circle" index="my-all" @click="$router.replace('/video/list?status=PASS')">已审</navigator-item>
    <navigator-item icon="hourglass_full" index="my-AUDITING" @click="$router.replace('/video/list?status=AUDITING')">待审</navigator-item>
    <navigator-item icon="error" index="my-REJECT" @click="$router.replace('/video/list?status=REJECT')">驳回</navigator-item>
