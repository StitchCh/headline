  ...
  <div class="flex-col recycle-center">
    ...
    <list-view :list="list" class="flex-item"
      ...
      <li slot-scope="slotProps">
        <div class="list-item c-6 f-14 a" :class="{ on: slotProps.item.id === id }" @click="$router.push('/ecommerce/recycle/' + slotProps.item.id)">
...
export default {
  ...
  methods: {
    getList (refresh) {
      ...
      this.$http.post('/cri-cms-platform/ecommerce/getRecycle.monitor', this.filter).then(res => {
        this.list = res.pages || []
    ...
    recover () {
      this.$confirm({
        ...
        color: 'green',
        yes: () => {
          this.$http.post('/cri-cms-platform/ecommerce/reduction.monitor', {
            selectedIds: this.id
          }).then(res => {
            this.getList()
            this.$router.replace('/ecommerce/recycle')
          })
        }
      })
    }
