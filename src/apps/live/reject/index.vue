<template>
  <div class="flex-item flex-col article-reject">
    ...
    <div class="flex-item flex-col">
      ...
      <div class="box flex-item relative scroll-y f-14">
        ...
        <ul class="flex" ref="ul" style="flex-wrap: wrap;padding-bottom: 50px;" :style="{paddingLeft: (width - (240 * ~~((width - 80) / 240))) * 0.5 + 'px'}">
          <li v-for="item in list" :key="item.id" class="a" @click="$router.push('/liveEdit/edit/' + item.id)">
  ...
  methods: {
    ...
    deleteItem (item) {
      ...
      this.$confirm({
        ...
        color: 'red',
        yes () {
          that.$http.post('/cri-cms-platform/live/delete.monitor', {id: item.id}).then(
            res => {
              that.getList()
            }
          )
    ...
    getList (refresh) {
      ...
      this.$http.post('/cri-cms-platform/live/list.monitor', this.filter).then(res => {
        this.list = res.pages || []
