    ...
    <div class="flex-item flex-col">
      <div class="af-topbar flex-v-center">
        <div v-if="$route.params.id" class="content-tool flex-v-center">
          ...
          <div class="tool-item">
            <icon-btn small v-tooltip:bottom="'编辑'" @click="openWindow(`/liveEdit/edit/${id}`)">edit</icon-btn>
          </div>
          <div class="tool-item">
...
export default {
  ...
  methods: {
    getLinkList (id) {
      this.$http.post('/cri-cms-platform/live/published.monitor', { id }).then(res => {
        this.urlList = res
    ...
    deleteArticle () {
      this.$confirm({
        ...
        color: 'red',
        yes: () => {
          this.$http.post('/cri-cms-platform/live/delete.monitor', {
