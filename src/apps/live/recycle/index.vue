  ...
  methods: {
    clearRecyle () {
      ...
      this.$confirm({
        ...
        color: 'red',
        yes () {
          othis.$http.post('/cri-cms-platform/live/clearRecyle.monitor', {
            id: othis.id
          }).then(res => {
            othis.getList()
            othis.$router.push({path: '/live/recycle'})
          })
        },
        no () {
    ...
    getList (refresh) {
      ...
      this.$http.post('/cri-cms-platform/live/getRecycle.monitor', this.filter).then(res => {
        this.list = res.pages || []
